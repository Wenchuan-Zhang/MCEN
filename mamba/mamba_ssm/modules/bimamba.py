...
class BiMamba(nn.Module):
    def __init__(
        ...
        self.d_inner = int(self.expand * self.d_model) #2*512=1024
        self.dt_rank = math.ceil(self.d_model / 16) if dt_rank == "auto" else dt_rank #32
        self.use_fast_path = use_fast_path #True
        self.layer_idx = layer_idx #none

        self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs) #512 >2048
        ...
        self.conv1d = nn.Conv1d(
            ...
            padding=d_conv - 1,
            **factory_kwargs,
        ) #Conv1d(1024, 1024, kernel_size=(4,), stride=(1,), padding=(3,), groups=1024)
        ...
        self.activation = "silu" #silu
        self.act = nn.SiLU()

        self.x_proj = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        ) #Linear(in_features=1024, out_features=64, bias=False)
        self.x_proj_b = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        ) #Linear(in_features=1024, out_features=64, bias=False)

        self.dt_proj = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs) #Linear(in_features=32, out_features=1024, bias=True)
        self.dt_proj_b = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs) #Linear(in_features=32, out_features=1024, bias=True)

        # Initialize special dt projection to preserve variance at initialization
        dt_init_std = self.dt_rank**-0.5 * dt_scale #0.1767
        if dt_init == "constant":
        ...
        dt = torch.exp(
            ...
            + math.log(dt_min)
        ).clamp(min=dt_init_floor) #1024
        # Inverse of softplus: https://github.com/pytorch/pytorch/issues/72759
        inv_dt = dt + torch.log(-torch.expm1(-dt)) #1024
        with torch.no_grad():
            self.dt_proj.bias.copy_(inv_dt) #Linear(in_features=32, out_features=1024, bias=True)
            self.dt_proj_b.bias.copy_(inv_dt)  # ?#Linear(in_features=32, out_features=1024, bias=True)
        ...
        A_log = torch.log(A)  # Keep A_log in fp32 #1024 * 16
        self.A_log = nn.Parameter(A_log) #1024 * 16
        ...
        A_b_log = torch.log(A_b)  # Keep A_b_log in fp32 #1024 * 16
        self.A_b_log = nn.Parameter(A_b_log)
        self.A_b_log._no_weight_decay = True 

        # D "skip" parameter
        self.D = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32 #1024
        self.D._no_weight_decay = True
        self.D_b = nn.Parameter(torch.ones(self.d_inner, device=device))  # Keep in fp32
        self.D_b._no_weight_decay = True

        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs) ##Linear(in_features=1024, out_features=512, bias=false)

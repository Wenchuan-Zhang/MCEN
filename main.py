...
def main(args):
    ...
    # --->generate dataset
    if args.datasets.lower() == 'pcrdatasets': 
    ...
    train_acs, train_pre, train_rec,train_fs,train_auc=[],[],[],[],[] 
    train_ckc_metric = [train_acs, train_pre, train_rec,train_fs,train_auc] 
    if not args.no_log:
    ...
    for k in range(args.fold_start, args.cv_fold):
        ...
        ckc_metric, train_ckc_metric= one_fold(conf,args,k,ckc_metric, train_ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l) 

    final_df = pd.DataFrame({ 
    'folds': list(range(args.fold_start, args.cv_fold)),
    'test_acc': acs,
    'test_pre': pre,
    'test_rec': rec,
    'test_f1': fs,
    ...
    combined_df.to_csv(os.path.join(args.model_path, "summary.csv"))
...
def one_fold(conf,args,k,ckc_metric, train_ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    # ---> Initialization
    seed_torch(args.seed)
    ...
    train_acs, train_pre, train_rec,train_fs,train_auc= train_ckc_metric
    # ---> Loading data
    if args.datasets.lower() == 'pcrdatasets':

        train_set = pCRDataset(train_p[k],train_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize,is_train=True)
        test_set = pCRDataset(test_p[k],test_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = pCRDataset(val_p[k],val_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        else:
            val_set = test_set
    ...
    if args.only_rrt_enc and args.model != 'rrtmil':
        model_params = {
            'region_num': args.region_num, # fixed better #8
            'drop_path': args.drop_path,   # fixed better #0
            'n_layers': args.n_trans_layers, # fixed better #2
            'attn': args.attn,             # fixed better #rmsa
            'epeg': args.epeg,             # fixed better #True
            'cr_msa': args.cr_msa,         # fixed better #True
            'all_shortcut': args.all_shortcut, # fixed better #True
            'crmsa_mlp':args.crmsa_mlp,    # fixed better #False
            'crmsa_heads':args.crmsa_heads,# fixed better #8
            'crmsa_k': args.crmsa_k,   # hyper-para [1,3,5] #3
            'epeg_k': args.epeg_k,     # hyper-para [9,15,21] #9
            'need_init': True
         }
    ...
    elif args.model == 'MCEN':
        model = MambaMIL(conf,type="BiMamba",input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act,rrt=rrt_enc).to(device)
    elif args.model == 'BiMamba':
        model = MambaMIL(conf,type="BiMamba",input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act,rrt=rrt_enc).to(device)
    elif args.model == 'SRMamba':
        model = MambaMIL(conf,type="SRMamba",input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act,rrt=rrt_enc).to(device)
    elif args.model == 'Mamba':
        model = MambaMIL(conf,type="Mamba",input_dim=args.input_dim,n_classes=args.n_classes,dropout=args.dropout,act=args.act,rrt=rrt_enc).to(device)
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience=30 if args.datasets=='pCRdatasets' else 20, stop_epoch=args.max_epoch if args.datasets=='pCRdatasets' else 70,save_best_model_stage=np.ceil(args.save_best_model_stage * args.num_epoch))
    else:
        early_stopping = None
    ...
    accuracy, auc_value, precision, recall, fscore,test_loss_log,test_patient_results = test(args,model,test_loader,device,criterion)
    train_accuracy, train_auc_value, train_precision, train_recall, train_fscore,train_test_loss_log,train_patient_results = test(args,model,train_loader,device,criterion) 
    save_pkl(os.path.join(args.model_path, f'split_{k}_train_results.pkl'), train_patient_results)
    save_pkl(os.path.join(args.model_path, f'split_{k}_val_results.pkl'), test_patient_results)
    if args.wandb:
        wandb.log({
            "test_acc":accuracy,
    ...
    for i, data in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.model in ('clam_sb','clam_mb','dsmil'):
                train_logits,cls_loss,patch_num = model(bag,label)
                keep_num = patch_num
            elif args.model in ('MCEN','SRMamba','BiMamba','Mamba'):
                train_logits, _, _, _ = model(bag)
...
def val_loop(args,model,loader,device,criterion,early_stopping,epoch):
    ...
    accuracy, auc_value, precision, recall, fscore = five_scores(bag_labels, bag_logit, not args.datasets.lower() == 'pcrdatasets')
...
def test(args,model,loader,device,criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if args.model in ('clam_sb','clam_mb','dsmil'):
                ...
                test_logits = model(bag)
            patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'prob': torch.softmax(test_logits,dim=-1).cpu().squeeze().numpy(), 'label': label.item()}}) 
            if args.loss == 'ce':
                if (args.model == 'dsmil' and args.ds_average) or (args.model == 'mhim' and isinstance(test_logits,(list,tuple))):
                    test_loss = criterion(test_logits[0].view(batch_size,-1),label)
                    bag_logit.append((0.5*torch.softmax(test_logits[1],dim=-1)+0.5*torch.softmax(test_logits[0],dim=-1))[:,1].cpu().squeeze().numpy())
    ...
    return accuracy, auc_value, precision, recall, fscore,test_loss_log,patient_results

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MCEN Training Script')

    # Dataset 
    parser.add_argument('--datasets', default='pCRdatasets', type=str, help='[pCRdatasets, tcga]')

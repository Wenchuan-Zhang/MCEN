...
class pCRDataset(Dataset):
    ...
    def __init__(self, file_name, file_label,root,persistence=False,keep_same_psize=0,is_train=False):
        ...
        :param transform: optional transform to be applied on a sample
        """
        super(pCRDataset, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            with h5py.File(file_path, 'r') as f_s: 
                features = torch.from_numpy(np.array(f_s['features']))  
                coords = torch.from_numpy(np.array(f_s['coords']))  
            

        label = int(self.slide_label[idx])


        return features , label, self.file_name[idx], coords 